...
module Api
  RSpec.describe EmployeeApi, type: :model do
    ...
    before do
      allow(Employees::Authorize).to receive(:token).and_return(token)
      stub_request(:get, /#{server_url}.*/).to_return(status: 200, body: '', headers: {})
    end
    ...
    describe '::load_user' do
      ...
      it 'returns instance of Faraday::Response class' do
        expect(subject.load_user(123)).to be_instance_of(Faraday::Response)
      end
    end

    describe '::load_users_by_id_tn' do
      it 'sends :get request with required params and headers' do
        subject.load_users_by_id_tn([123])
      ...
      it 'returns instance of Faraday::Response class' do
        expect(subject.load_users_by_id_tn([123])).to be_instance_of(Faraday::Response)
      end
    end

    describe '::load_users_by_tn' do
      it 'sends :get request with required params and headers' do
        subject.load_users_by_tn([123])
      ...
      it 'returns instance of Faraday::Response class' do
        expect(subject.load_users_by_tn([123])).to be_instance_of(Faraday::Response)
      end
    end
