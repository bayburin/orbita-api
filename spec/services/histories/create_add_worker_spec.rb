...
# module Histories
#   RSpec.describe CreateAddWorkers do
#     let(:event_type) { create(:event_type, :add_self) }
#     let(:user) { create(:admin) }
#     let(:claim) { create(:claim) }
#     let(:work) { create(:work, claim: claim) }
#     let(:event_dbl) { double(:event, event_type: event_type, user: user, claim: claim, work: work, payload: { workers: 'workerse list' }.as_json) }
#     subject(:context) { described_class.call(event: event_dbl) }

#     describe '.call' do
#       it { expect(context).to be_a_success }
#       it { expect { context }.to change { History.count } }
#       it { expect(context.history.action).to eq event_type.template }
