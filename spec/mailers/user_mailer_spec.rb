...
RSpec.describe UserMailer, type: :mailer do
  ...
  describe '#sd_request_updated_email' do
    let(:sd_request) { create(:sd_request) }
    let(:mail) { described_class.sd_request_updated_email(recipient, sd_request) }
    let(:sender) { ENV['ACTION_MAILER_USERNAME'] }
    ...
    it 'render the subject' do
      expect(mail.subject).to eq("Портал \"Орбита\": внесены изменения в заявку №#{sd_request.id}")
    end
